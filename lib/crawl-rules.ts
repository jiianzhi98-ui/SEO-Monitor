...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '去重', text: '按 (site_id, url) 唯一索引 upsert；新页面写入 first_seen_date=today（DB trigger 保护，UPDATE 时不覆盖）；已知页面更新 last_seen_date=today；抓完后将 last_seen_date < 60天前 的老页面标记 disappeared_date=today（60天阈值：避免因 Baidu 月度结果轮换而误判当月旧页脱收）；重新出现则清 disappeared_date 为 null' },
