...
export const CRAWL_RULES: RuleSection[] = [
  {
    key: 'keywords',
    title: '关键词抓取',
    badge: 'step=keywords · GitHub Actions · 目标 03:00 MYT（cron 22:00 MYT + 排队约 5h）',
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 14 * * * UTC = 22:00 MYT 前一天)，5个 matrix job 并行，每组抓约1/5的站点；实际执行脚本：scripts/crawl.ts（非 /api/cron，两条路径）；GitHub runner 排队约5小时，实际执行约 03:00 MYT。失败/空站由 retry-crawl.yml (cron 0 20 UTC = 04:00 MYT) 在约 09:00 MYT 自动补抓' },
  ...
  },
  {
    ...
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 15 * * * UTC = 23:00 MYT 前一天)，5个 matrix job 并行；实际执行脚本：scripts/crawl.ts；实际执行约 04:00 MYT。失败站由 retry-crawl.yml (cron 30 20 UTC = 04:30 MYT) 在约 09:30 MYT 自动补抓' },
  ...
  },
  {
    ...
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 16 * * * UTC = 00:00 MYT 前一天)，5个 matrix job 并行；实际执行脚本：scripts/crawl.ts；实际执行约 05:00 MYT。失败/空站由 retry-crawl.yml (cron 0 21 UTC = 05:00 MYT) 在约 10:00 MYT 自动补抓' },
  ...
  },
  {
    key: 'index-pages',
    title: '收录页面追踪',
    badge: 'step=index-pages · GitHub Actions · 目标 08:00 MYT（cron 00:00 UTC）',
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 0 * * * UTC = 08:00 MYT)，5个 matrix job 并行；支持页面手动重抓 → /api/trigger-crawl → /api/cron?step=index-pages' },
  ...
  },
  {
    key: 'rank-positions',
    title: '自有站点排名位置',
    badge: 'step=rank-positions · GitHub Actions · 目标 07:30 MYT（cron 23:30 UTC）',
    items: [
      { label: '触发方式', text: 'GitHub Actions rank-positions.yml (cron 30 23 * * * UTC = 07:30 MYT)，单 job，无 matrix 分组；脚本：scripts/crawl-rank-positions.ts' },
  ...
  },
  {
    key: 'rank-title',
    title: '竞品排名+标题抓取',
    badge: 'step=rank-title · GitHub Actions · 目标 07:00 MYT（cron 23:00 UTC）',
    items: [
      { label: '触发方式', text: 'GitHub Actions rank-title-crawl.yml (cron 0 23 * * * UTC = 07:00 MYT)，单 job，无 matrix 分组；脚本：scripts/crawl-rank-title.ts' },
