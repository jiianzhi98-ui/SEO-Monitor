...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '数据来源', text: '爱站 baidurank.aizhan.com，移动端（/mobile/）+ PC端（/baidu/），各抓涨入和跌出，共 4 个组合；移动端含标题（title）和排名页 URL；PC端无标题，仅关键词+排名+搜索量' },
      { label: '并行策略', text: '排名段 1-5 同时并行，段内按页顺序，每页间隔 300ms；4 个组合顺序执行，中间等待 2s；站点间间隔 45s' },
      { label: '翻页上限', text: '每段最多 15 页；抓取全部词（不过滤 volume=0）' },
      { label: '写入表', text: 'site_rank_keywords（永久保留，按 site_id+keyword+stat_date+type+platform 唯一，每次运行先删当日全部记录再插入）/ keyword_volume（仅移动端 rankup 且 volume>0 的词，upsert）' },
