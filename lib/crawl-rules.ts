...
export const CRAWL_RULES: RuleSection[] = [
  {
    ...
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 13 * * * UTC = 21:00 MYT 前一天)，5个 matrix job 并行，每组抓约1/5的站点；实际执行脚本：scripts/crawl.ts（非 /api/cron，两条路径）；GitHub runner 排队约5小时，实际执行约 02:00 MYT。失败/空站由 retry-crawl.yml (cron 0 17 UTC = 01:00 MYT) 在约 06:00 MYT 自动补抓' },
  ...
  },
  {
    ...
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 14 * * * UTC = 22:00 MYT 前一天)，5个 matrix job 并行；实际执行脚本：scripts/crawl.ts；实际执行约 03:00 MYT。失败站由 retry-crawl.yml (cron 30 17 UTC = 01:30 MYT) 在约 06:30 MYT 自动补抓' },
  ...
  },
  {
    ...
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 15 * * * UTC = 23:00 MYT 前一天)，5个 matrix job 并行；实际执行脚本：scripts/crawl.ts；实际执行约 04:00 MYT。失败/空站由 retry-crawl.yml (cron 0 18 UTC = 02:00 MYT) 在约 07:00 MYT 自动补抓' },
  ...
  },
  {
    key: 'rank-positions',
    title: '自有站点排名位置',
    badge: 'step=rank-positions · GitHub Actions · 目标 06:00 MYT（cron 22:00 UTC）',
    items: [
      { label: '触发方式', text: 'GitHub Actions rank-positions.yml (cron 0 22 * * * UTC = 06:00 MYT)，单 job，无 matrix 分组；脚本：scripts/crawl-rank-positions.ts' },
