...
export const CRAWL_RULES: RuleSection[] = [
  {
    ...
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 15 * * * UTC = 23:00 MYT 前一天)，5个 matrix job 并行，每组抓约1/5的站点；实际执行脚本：scripts/crawl.ts（非 /api/cron，两条路径）；GitHub runner 排队约1小时，实际执行约 00:00 MYT。失败/空站由 retry-crawl.yml (cron 45 21 UTC = 05:45 MYT) 自动补抓' },
  ...
  },
  {
    ...
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 16 * * * UTC = 00:00 MYT 当天)，5个 matrix job 并行；实际执行脚本：scripts/crawl.ts；实际执行约 01:00 MYT。失败站由 retry-crawl.yml (cron 0 22 UTC = 06:00 MYT) 自动补抓' },
  ...
  },
  {
    ...
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 0 17 * * * UTC = 01:00 MYT 当天)，5个 matrix job 并行；实际执行脚本：scripts/crawl.ts；实际执行约 02:00 MYT。失败/空站由 retry-crawl.yml (cron 15 22 UTC = 06:15 MYT) 自动补抓' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '抓取方式', text: '百度 site:domain 搜索，带近31天时间过滤（gpc=stf={now-31d},{now}|stftype=1 + tfflag=1；不带 ct=2097152/si= 以避免服务器IP被限页），pn=0/10/20... 翻页，无页数上限；停止条件：空页、被拦截、或当前页 URL 集合完全包含于上一页（整页相同则立即停）；页间延迟 1.5-3 秒（有 Cookie），4-7 秒（无 Cookie）；站间延迟 10 秒' },
  ...
  },
  {
    key: 'rank-positions',
    title: '自有站点排名位置',
    badge: 'step=rank-positions · GitHub Actions · 目标 03:30 MYT（cron 19:30 UTC）',
    items: [
      { label: '触发方式', text: 'GitHub Actions rank-positions.yml (cron 30 19 * * * UTC = 03:30 MYT)，单 job，无 matrix 分组；脚本：scripts/crawl-rank-positions.ts' },
  ...
  },
  {
    key: 'rank-title',
    title: '竞品排名+标题抓取',
    badge: 'step=rank-title · GitHub Actions · 目标 02:00 MYT（cron 18:00 UTC）',
    items: [
      { label: '触发方式', text: 'GitHub Actions rank-title-crawl.yml (cron 0 18 * * * UTC = 02:00 MYT)，单 job，无 matrix 分组；脚本：scripts/crawl-rank-title.ts' },
