...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '抓取方式', text: '百度 site:domain 搜索，每个站点依次跑三个 gpc 时间窗口：月(31天)、周(7天)、日(1天)（gpc=stf={now-Nd},{now}|stftype=1 + tfflag=1），过滤依据是百度内部重新收录时间；三轮结果按 URL 去重合并，取并集以获得最完整的收录页面列表；带 ct=2097152/si=domain/fenlei=256 开启百度站内搜索模式；pn=0/10/20... 翻页，无页数上限；停止条件：空页、被拦截（captcha 则中止当站剩余轮次）、或整页URL相同；周期间延迟 3 秒，站间延迟 10 秒；SUPPLEMENT_PERIOD 环境变量可覆盖为单一周期（manual/supplement 专用）' },
  ...
  },
  {
    key: 'tracking',
    title: '成效追踪（竞品 + 自己站点）',
    badge: 'step=tracking · GitHub Actions · 06:15 MYT（cron 22:15 UTC，rank-title retry 完成后）',
    items: [
      { label: '触发方式', text: 'GitHub Actions daily-crawl.yml (cron 15 22 * * * UTC = 06:15 MYT)，在所有主抓取和重试（含 rank-title retry 06:00 MYT）完成后运行；脚本：scripts/crawl.ts --step=tracking；不设 retry，因为记录是持久化的，漏一天次日补跑即可' },
