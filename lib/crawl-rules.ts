...
export const CRAWL_RULES: RuleSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '抓取方式', text: '百度 site:domain 搜索，带近31天时间过滤（ct=2097152 + gpc=stf={now-31d},{now}|stftype=1），pn=0/10/20... 翻页，无页数上限；停止条件：空页、被拦截、或当前页 URL 集合完全包含于上一页（整页相同则立即停）；页间延迟 1.5-3 秒（有 Cookie），4-7 秒（无 Cookie）；站间延迟 10 秒' },
